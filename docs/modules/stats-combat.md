...
hpRegen     = con*0.05 + flatBonus.hpRegen
...
确保升级 INT/AGI 也能提升对应职业的攻击力。`bonusPct.attack`（装备百分比攻击加成）也正确应用。

`CharacterPanel` 显示同步使用 `player.getAttack()`，避免与实战不一致。属性 tooltip 根据职业主属性动态显示攻击力贡献（如射手 AGI 显示"攻击（主属性）"）。
